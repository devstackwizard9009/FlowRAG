const ACTIVE_ENGINE_STORAGE_KEY = "flowrag_active_engines";
...
const UI_LANGUAGE_STORAGE_KEY = "flowrag_ui_language";
...
const WORKSPACE_PANE_WIDTH_KEY = 'flowrag_workspace_pane_width';
...
const CHUNK_CONFIG_STORAGE_KEY = "flowrag_chunk_config";
const INDEX_CONFIG_STORAGE_KEY = "flowrag_index_config";
...
function saveCurrentSession(force = false) {
    ...
    if (!hasContent) {
        ...
        renderChatSidebar();
        localStorage.setItem("flowrag_sessions", JSON.stringify(state.chat.sessions));
        return;
    }
    ...
    if (!contentChanged) {
        if (state.chat.currentSessionId) {
            localStorage.setItem("flowrag_last_active_id", state.chat.currentSessionId);
        }
        return;
    }
...
async function deleteChatSession(sessionId) {
    ...
    if (wasCurrent) {
        localStorage.removeItem("flowrag_last_active_id"); // Clear
...
async function deleteAllChatSessions() {
    ...
    state.chat.sessions = [];
    localStorage.setItem("flowrag_sessions", JSON.stringify(state.chat.sessions));
    localStorage.removeItem("flowrag_last_active_id");
    createNewChatSession();
}
...
function bindEvents() {
    ...
    if (els.chatSidebarToggleBtn && els.chatSidebar) {
        els.chatSidebarToggleBtn.onclick = () => {
            ...
            localStorage.setItem("flowrag_sidebar_collapsed", isCollapsed);
        };
    }
...
async function bootstrap() {
    ...
    const wasCollapsed = localStorage.getItem("flowrag_sidebar_collapsed") === "true";
    ...
    const savedSessions = localStorage.getItem("flowrag_sessions");
    if (savedSessions) {
        try {
            ...
            const lastId = localStorage.getItem("flowrag_last_active_id");
            if (lastId) {
...
const USER_ID_STORAGE_KEY = 'flowrag_user_id';
...
const BG_TASKS_STORAGE_KEY = 'flowrag_background_tasks';
...
const AI_HISTORY_KEY = 'flowrag_ai_history';
...
const AI_WELCOME_HTML = `
    <div class="ai-welcome">
        ...
        <h4 data-i18n="builder_ai_welcome_title">FlowRAG AI Assistant</h4>
...
function loadAISettings() {
    try {
        const saved = localStorage.getItem('flowrag_ai_settings');
        if (saved) {
...
function saveAISettings(options = {}) {
    ...
    try {
        localStorage.setItem('flowrag_ai_settings', JSON.stringify(aiState.settings));
