...
class _CallWrapper:
    ...
    async def _ensure_client(self):
        ...
        if _client is None:
            raise RuntimeError(
                "[FlowRAG Error] ToolCall was used before `initialize()` was called."
            )
        try:
            _ = _client.session
        except RuntimeError:
    ...
    async def _async_call(self, *args, **kwargs):
        ...
        with open(os.path.join(SERVER_ROOT, self._server, "server.yaml"), "r") as f:
            try:
                ...
                raise ValueError(
                    f"[FlowRAG Error] Tool {self._tool} not found in server {self._server} configuration!"
                ) from e
        ...
        if len(args) > len(input_param):
            raise ValueError(
                f"[FlowRAG Error] Expected at most {len(input_param)} positional args, got {len(args)}"
            )
        ...
        for k, v in kwargs.items():
            if k not in input_param:
                raise ValueError(f"[FlowRAG Error] Unexpected keyword arg: {k!r}")
            input_param[k] = v

        missing = [k for k, v in input_param.items() if v is None]
        if missing:
            raise ValueError(f"[FlowRAG Error] Missing value for key(s): {missing}")
...
async def _pipeline_async(
    ...
    """Internal async helper that runs a full FlowRAG pipeline.
...
def PipelineCall(
    ...
    """Run a full FlowRAG pipeline from Python.

    Similar to `flowrag run`, but with an explicitly provided parameter file.
