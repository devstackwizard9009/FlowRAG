...
def get_logger(
    ...
    Args:
        name: Logger name (child logger of "FlowRAG" if not "FlowRAG")
    ...
    base = logging.getLogger("FlowRAG")
    ...
    if not _LOGGING_INITIALIZED:
        ...
        if log_file:
            ...
            ts = os.environ.get("FLOWRAG_LOG_TS") or datetime.now().strftime(
                "%Y%m%d_%H%M%S"
            )
    ...
    return base if name == "FlowRAG" else base.getChild(name)
