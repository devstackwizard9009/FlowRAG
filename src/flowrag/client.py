...
from flowrag.cli import log_server_banner
from flowrag.mcp_exceptions import (
    NodeNotInstalledError,
    NodeVersionTooLowError,
    check_node_version,
)
from flowrag.mcp_logging import get_logger
...
def launch_ui(
    ...
    ui_logger = logging.getLogger("FlowRAG-UI")
    mode_str = "Admin" if admin_mode else "Chat"
    ui_logger.info("FlowRAG UI (%s mode) started: http://%s:%d", mode_str, host, port)

    try:
        app.run(host=host, port=port, debug=False)
    except OSError as exc:
        raise RuntimeError(
            f"Failed to start FlowRAG UI (host={host}, port={port}): {exc}"
        ) from exc
...
class UltraData:
    """Data manager for FlowRAG pipeline execution.
        ...
        for i, elem in enumerate(skeleton):
            if elem_match(elem, parent_pairs):
                try:
                    ...
                    raise ValueError(
                        "[FlowRAG Error] Router sub_list length < expected matches when padding to skeleton"
                    )
    ...
    def _extract_io(self, pipeline: List[Any]) -> None:
        ...
        for pipe in pipeline:
            if isinstance(pipe, str):
                ...
                for _, input_val in tool_input.items():
                    if input_val.startswith("$"):
                        ...
                        if stripped not in self.local_vals[srv_name]:
                            raise ValueError(
                                f"[FlowRAG Error] Variable {stripped} not found in {srv_name} parameter.yaml"
                            )
                        ...
                        if mem_name.startswith("memory_"):
                            ...
                            if input_val not in self.global_vars.keys():
                                raise ValueError(
                                    f"[FlowRAG Error] Variable {input_val} cannot be found from pipeline before {srv_name}.{tool_name} step"
                                )
                ...
                for output_val in tool_output:
                    if output_val.startswith("$"):
                        ...
                        if not output_val in self.local_vals[srv_name]:
                            raise ValueError(
                                f"[FlowRAG Error] Variable {output_val} not found in {srv_name} parameter.yaml"
                            )
                ...
                raise ValueError(f"[FlowRAG Error] Unrecognized pipeline step: {pipe}")
                    ...
                    for i, elem in enumerate(full_list):
                        if elem_match(elem, parent_pairs):
                            try:
                                ...
                                raise ValueError(
                                    f"[FlowRAG Error] Prompt {var_name} length < "
            ...
            if len(output_keys) > 0:
                ...
                for key in iter_keys:
                    if not key.replace("$", "") in data:
                        raise ValueError(
                            f"[FlowRAG Error] Output key {key} not found in data for step {server_name}.{tool_name}"
                        )
                    if key.startswith("$"):
                        if key[1:] in self.local_vals[server_name]:
                            ...
                            raise ValueError(
                                f"[FlowRAG Error] Variable {key[1:]} not found in {server_name} parameter.yaml"
                            )
                                ...
                                for i, elem in enumerate(full_list):
                                    if elem_match(elem, parent_pairs):
                                        try:
                                            ...
                                            raise ValueError(
                                                f"[FlowRAG Error] Router {key} length < global_vars in step {server_name}.{tool_name}"
                                            )
                                        full_list[i]["data"] = new_elem
                                if any(True for _ in it):
                                    raise ValueError(
                                    ...
                                    for i, elem in enumerate(full_list):
                                        if elem_match(elem, parent_pairs):
                                            try:
                                                ...
                                                raise ValueError(
                                                    f"[FlowRAG Error] Router {key} length < global_vars"
                                                )
        ...
        if path.endswith(".py"):
            if not os.path.exists(path):
                raise FileNotFoundError(
                    f"[FlowRAG Error] Cannot find the server file of {name}: {path}"
                )
            mcp_servers[name] = {
                "command": "python",
                "args": [path],
            ...
            if not node_status:
                try:
                    ...
                    logger.error(
                        "[FlowRAG Error] Node.js is not installed or not found in PATH. Please install Node.js >= 20."
                    )
                    logger.error(str(e))
                    sys.exit(1)
                except NodeVersionTooLowError as e:
                    logger.error(
                        "[FlowRAG Error] Node.js version is too low. Please upgrade to Node.js >= 20."
                    )
                    logger.error(str(e))
                    sys.exit(1)
            mcp_servers[name] = (
            ...
            raise ValueError(
                f"[FlowRAG Error] Unsupported server type for {name}: {path}"
            )
        ...
        for step in steps:
            if isinstance(step, str):
                ...
                if "loop" in step:
                    ...
                    raise ValueError(
                        f"[FlowRAG Error] Unrecognized step in branch: {step}"
                    )
            else:
                raise ValueError(f"[FlowRAG Error] Unrecognized pipeline step: {step}")
...
def load_pipeline_context(
    ...
    if param_path is not None:
        ...
        if param_config_path is None:
            raise FileNotFoundError(
                f"[FlowRAG Error] Parameter file '{provided_path}' does not exist"
            )
...
async def execute_pipeline(
    ...
    """Execute FlowRAG pipeline with given context.
...
async def run(
    ...
    """Run FlowRAG pipeline with given configuration.
...
def main() -> None:
    """Main entry point for FlowRAG CLI."""
    parser = argparse.ArgumentParser(prog="flowrag", description="FlowRAG CLI")
    ...
    p_show_ui = show_sub.add_parser("ui", help="Launch the FlowRAG web UI")
